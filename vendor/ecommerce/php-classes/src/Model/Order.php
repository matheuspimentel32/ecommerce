...
class Order extends Model {
	...
	public function save()
	{
        ...
        $results = $sql->select("INSERT INTO tb_orders (idorder, idcart, iduser, idstatus, idaddress, vltotal)
	...
	public function updateStatus()
	{
		...
		$results = $sql->select("UPDATE `tb_orders` SET `idstatus` = :idstatus WHERE `idorder` = :idorder", [
			':idorder'=>$this->getidorder(),
            ':idstatus'=>$this->getidstatus(),
		]);
    ...
    public static function getPageSearch($search, $page = 1, $itemsPerPage = 10)
	{
		...
		$results = $sql->select("SELECT SQL_CALC_FOUND_ROWS *
			...
			INNER JOIN tb_users d ON d.iduser = a.iduser
			INNER JOIN tb_persons f ON f.idperson = d.idperson
			WHERE a.idorder = :searchId OR f.desperson LIKE :search
			ORDER BY a.dtregister DESC
            LIMIT $start, $itemsPerPage;
		", [
			':search'=>'%' . $search . '%',
